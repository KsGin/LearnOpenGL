...
float FOV = 60.0f;
float lastX = static_cast<float>((float) WIDTH / 2.0);
float lastY = static_cast<float>((float) HEIGHT / 2.0);
...
int main() {
    ...
    glm::mat4 projection = glm::perspective(glm::radians(120.0f), (float) WIDTH / (float) HEIGHT, 0.1f, 100.0f);
